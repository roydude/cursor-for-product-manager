...
### git_branch_management.mdc

- Agent를 활용한 깃 협업 가이드
- 비개발직군이 자연어로 요청하면 Agent가 깃 작업 대신 수행
- 모든 작업은 feature 브랜치로 진행하여 안정성 보장
- PR을 통한 병합으로 충돌 방지 및 이력 관리
...
1. **참고 문서로 활용**:
   ...
   ```
   @mdc/git_branch_management.mdc
   ```

2. **핵심 개념**:
   - **Agent 활용**: 터미널 명령어 대신 자연어로 요청
   - **feature 브랜치 필수**: 모든 작업은 feature 브랜치에서 시작
   - **선택적 리뷰**: 간단한 수정은 셀프 머지, 중요한 작업은 리뷰 요청
...
최신 버전으로 업데이트해 줘
feature 브랜치 만들어 줘
브랜치명: feature/홍길동-PRD오타수정

# 작업 및 푸시
작업 완료했어. 커밋하고 푸시해 줘
커밋 메시지: [수정] 메인 페이지 PRD 오타 수정

# GitHub에서 PR 생성 후 셀프 머지 또는 리뷰 요청
...
**모든 작업 → feature 브랜치 필수**

- 간단한 수정: PR 생성 후 셀프 머지
- 중요한 작업: PR 생성 후 리뷰 요청
- 안정성을 위해 main 브랜치 직접 수정 금지

#### 특징

- **자연어 기반**: 복잡한 git 명령어 불필요
- **안정성 우선**: 모든 변경사항은 PR을 통해 검증
- **선택적 리뷰**: 작업 중요도에 따라 셀프 머지 또는 리뷰 요청
- **시즌별 운영**: 기획 시즌과 유지보수 시즌 구분
